...
Нужно для того, чтобы из View передавать на фронтэнд всплывающие сообщения (toasts).
Работает через htmx.

For each request made with HTMX (detected with the HX-Request header), this middleware
puts all the messages into the HX-Trigger header of the response. For example, this header will contain:
